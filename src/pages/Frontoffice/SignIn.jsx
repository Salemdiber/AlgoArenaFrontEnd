...
const SignIn = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            // Call auth endpoint directly so we can detect 2FA requirement
            const resp = await authService.login({ username, password, recaptchaToken });
            if (resp && resp.two_factor_required) {
                const fallbackPath = '/';
                const fromPath = location.state?.from?.pathname || fallbackPath;
                // Pass username both in navigation state and as query param to ensure availability
                navigate(`/two-factor?username=${encodeURIComponent(username)}`, { state: { username, from: fromPath } });
                return;
            }

            // On success the backend should have set tokens; refresh local auth state
            await reload();

            // Get fresh profile to decide redirect destination
            const profile = await userService.getProfile('me');

            // If user's profile indicates 2FA is enabled, send them to the TwoFactor page
            const has2fa = profile?.two_factor_enabled || profile?.twoFactorEnabled || profile?.twoFAEnabled || profile?.two_factor || false;
            if (has2fa) {
                const fallbackPath = '/';
                const fromPath = location.state?.from?.pathname || fallbackPath;
                navigate('/two-factor', { state: { username, from: fromPath } });
                return;
            }

            const fallbackPath = redirectBasedOnRole(profile);
            ...
            toast({ title: 'Sign in failed', description: err?.message || 'Unable to sign in', status: 'error', duration: 4000, isClosable: true });
        } finally {
            setIsLoading(false);
        }
    };
