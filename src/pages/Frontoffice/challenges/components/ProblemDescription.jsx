...
const ProblemDescription = () => {
    ...
    return (
        <VStack spacing={6} align="stretch">
            ...
            <Box>
                <HStack spacing={3} mb={4}>
                    ...
                    {selectedChallenge.tags.map(tag => (
                        <Tag key={tag} bg="gray.700" color="gray.300" size="sm" borderRadius="8px" fontSize="xs">
                            {tag}
                        </Tag>
                    ))}
                </HStack>

                <Text fontFamily="heading" fontSize="3xl" fontWeight="bold" color="gray.100" mb={3}>
                ...
                <HStack spacing={6} fontSize="sm" color="gray.400">
                    ...
                    <Text>
                        Acceptance: <Text as="strong" color="gray.100">{selectedChallenge.acceptanceRate}%</Text>
                    </Text>
                </HStack>
            </Box>
            ...
            {/* Problem statement */}
            <Box>
                <Text fontFamily="heading" fontWeight="bold" color="gray.100" mb={3}>
                    Problem Statement
                </Text>
                {selectedChallenge.description.split('\n').filter(Boolean).map((para, i) => (
                    <Text key={i} color="gray.300" lineHeight="1.8" mb={3}>
            ...
            {selectedChallenge.examples.map((ex, i) => (
                <Box key={i}>
                    ...
                    <Box bg="#1e293b" borderRadius="12px" p={4} fontFamily="mono" fontSize="sm">
                        <Box mb={2}>
                            <Text as="span" color="gray.400">Input: </Text>
                            <Text as="span" color="gray.100">{ex.input}</Text>
                        </Box>
                        <Box mb={ex.explanation ? 2 : 0}>
                            <Text as="span" color="gray.400">Output: </Text>
                            <Text as="span" color="gray.100">{ex.output}</Text>
                        </Box>
                        {ex.explanation && (
                            <Box>
                                <Text as="span" color="gray.400">Explanation: </Text>
                                <Text as="span" color="gray.300">{ex.explanation}</Text>
                            </Box>
                        )}
                    </Box>
                </Box>
                    ...
                    {selectedChallenge.constraints.map((c, i) => (
                        <Flex key={i} align="flex-start" gap={2}>
                            ...
                            {c.includes('<=') || c.includes('==') ? (
                                <Code bg="gray.800" px={2} py={1} borderRadius="md" color="brand.500" fontSize="sm">
                                    {c}
                                </Code>
                            ) : (
                ...
                <Box>
                    ...
                    <VStack spacing={2} align="stretch">
                        {selectedChallenge.hints.map((hint, i) => (
                            <Box key={i} bg="#1e293b" borderRadius="12px" overflow="hidden">
                                <Button
                                    w="full"
